...
namespace GUI
{
    public partial class Form1 : Form
    {
        ...
        private void buttonIncarcarePoza_Click(object sender, EventArgs e)
        {
            ...
            if (ip.PhotoExist(cale) == 1)
            {
                textBox2.Text = ip.GetEveniment(cale);
                textBox3.Text = ip.GetLoc(cale);
                textBox1.Text = ip.GetNume(cale);
                dateTimePicker1.Value = ip.GetDataCrearii(cale);
                if (ip.GetPersoane(cale) is null)
                {
                    listBox2.Items.Clear();
                }
                else
                {
                    persoane = ip.GetPersoane(cale).Split(',');
        ...
        private void buttonSalvare_Click(object sender, EventArgs e)
        {
            if (pictureBox1.ImageLocation != null)
            {
                ...
                ip.AddPhoto(pictureBox1.ImageLocation, dateTimePicker1.Value.Date, textBox2.Text, textBox3.Text, persoane);
                listBox1.Items.Clear();
                string[] lista = ip.GetNames();
                foreach (var s in lista)
                {
            ...
            else if (axWindowsMediaPlayer1.URL != null)
            {
                ...
                ip.AddPhoto(axWindowsMediaPlayer1.URL, dateTimePicker1.Value.Date, textBox2.Text, textBox3.Text, persoane);
                listBox1.Items.Clear();
                string[] lista = ip.GetNames();
                foreach (var s in lista)
                {
        ...
        private void listBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (extensii_poze.Contains(cale.Split('.').Last()))
            {
                if (!(listBox1.SelectedItem is null))
                {
                    ...
                    string[] persoane;
                    pictureBox1.ImageLocation = listBox1.SelectedItem.ToString();
                    textBox2.Text = ip.GetEveniment(pictureBox1.ImageLocation);
                    textBox3.Text = ip.GetLoc(pictureBox1.ImageLocation);
                    textBox1.Text = ip.GetNume(pictureBox1.ImageLocation);
                    dateTimePicker1.Value = ip.GetDataCrearii(pictureBox1.ImageLocation);
                    if (ip.GetPersoane(pictureBox1.ImageLocation) is null)
                    {
                        listBox2.Items.Clear();
                    }
                    else
                    {
                        listBox2.Items.Clear();
                        persoane = ip.GetPersoane(pictureBox1.ImageLocation).Split(',');
                        for (int i = 0; i < persoane.Length; i++)
                        {
                            listBox2.Items.Add(persoane[i]);
                        }
            ...
            else if (extensii_video.Contains(cale.Split('.').Last()))
            {
                if (!(listBox1.SelectedItem is null))
                {
                    ...
                    string[] persoane;
                    axWindowsMediaPlayer1.URL = listBox1.SelectedItem.ToString();
                    textBox2.Text = ip.GetEveniment(axWindowsMediaPlayer1.URL);
                    textBox3.Text = ip.GetLoc(axWindowsMediaPlayer1.URL);
                    textBox1.Text = ip.GetNume(axWindowsMediaPlayer1.URL);
                    dateTimePicker1.Value = ip.GetDataCrearii(axWindowsMediaPlayer1.URL);
                    if (ip.GetPersoane(axWindowsMediaPlayer1.URL) is null)
                    {
                        listBox2.Items.Clear();
                    }
                    else
                    {
                        listBox2.Items.Clear();
                        persoane = ip.GetPersoane(axWindowsMediaPlayer1.URL).Split(',');
                        for (int i = 0; i < persoane.Length; i++)
                        {
                            listBox2.Items.Add(persoane[i]);
                        }
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            if(!(comboBox1.SelectedItem is null))
            {
                ...
                {
                    listBox1.Items.Clear();
                    string[] lista = ip.GetEvenimentList(textBox4.Text);
                    foreach (var s in lista)
                    {
                        listBox1.Items.Add(s);
                    }
                ...
                {
                    listBox1.Items.Clear();
                    string[] lista = ip.GetPersoanaList(textBox4.Text);
                    foreach (var s in lista)
                    {
                        listBox1.Items.Add(s);
                    }
                ...
                {
                    listBox1.Items.Clear();
                    string[] lista = ip.GetLocList(textBox4.Text);
                    foreach (var s in lista)
                    {
                        listBox1.Items.Add(s);
                    }
                ...
                {
                    listBox1.Items.Clear();
                    string[] lista = ip.GetDataCreariiList(textBox4.Text);
                    foreach (var s in lista)
                    {
                        listBox1.Items.Add(s);
                    }
        ...
        {
            if(pictureBox1.ImageLocation != null && ip.PhotoExist(pictureBox1.ImageLocation) != 0)
            {
                ip.DeletePhoto(pictureBox1.ImageLocation);
                listBox1.Items.Clear();
                string[] lista = ip.GetNames();
                foreach (var s in lista)
                {
            ...
            else if(axWindowsMediaPlayer1.URL != null && ip.PhotoExist(axWindowsMediaPlayer1.URL) != 0)
            {
                ip.DeletePhoto(axWindowsMediaPlayer1.URL);
                listBox1.Items.Clear();
                string[] lista = ip.GetNames();
                foreach (var s in lista)
                {
